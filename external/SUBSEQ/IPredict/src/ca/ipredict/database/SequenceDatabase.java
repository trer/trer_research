...
public class SequenceDatabase {
	...
	public void normaliseSequenceData(){
		SortedMap<Integer, Integer> mapSequenceData = new TreeMap<Integer, Integer>();
		...
		for (Map.Entry<Integer,Integer> entry : mapSequenceData.entrySet()){
			...
			String str = new Character((char) tmp).toString();
	...
	public void loadFileLargeTextFormatAsCharacter(String filepath, int maxCount, int minSize, int maxSize) throws IOException {
		...
		try {
			...
			Sequence sequence = new Sequence(-1);
			sequence.addItem(new Item((int) a1));
			sequence.addItem(new Item((int) a2));
			sequence.addItem(new Item((int) a3));
			sequence.addItem(new Item((int) a4));
			sequence.addItem(new Item((int) a5));
			sequence.addItem(new Item((int) a6));
			sequence.addItem(new Item((int) a7));
			sequence.addItem(new Item((int) a8));
			sequences.add(sequence);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
